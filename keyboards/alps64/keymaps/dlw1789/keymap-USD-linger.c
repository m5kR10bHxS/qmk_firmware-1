...
const custom_shift_key_t custom_shift_keys[] = {
    ...
    {TH_1,    LCA(KC_F1)},  // Shift 1 is TTY1
    {TH_2,    LCA(KC_F2)},  // Shift 2 is TTY2
    {TH_3,    LCA(KC_F3)},  // Shift 3 is TTY3
    {TH_4,    LCA(KC_F4)},  // Shift 4 is TTY4
    {TH_5,    LCA(KC_F5)},  // Shift 5 is TTY5
    {TH_6,    LCA(KC_F6)},  // Shift 6 is TTY6
    {TH_7,    LCA(KC_F7)},  // Shift 7 is TTY7
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [_TITANIUM] = LAYOUT_infinity( \
    ...
    XXXXXXX, TH_X,    TH_F,    KC_L,    KC_D,    TH_W,    TH_TILD, KC_U,    KC_O,    TH_Y,    TH_B,    XXXXXXX, XXXXXXX, XXXXXXX,          \
    XXXXXXX, TH_C,    S_GUI,   N_ALT,   T_CTL,   TH_K,    TH_COMM, A_CTL,   E_ALT,   I_GUI,   TH_H,    XXXXXXX, XXXXXXX,                   \
    XXXXXXX, KC_J,    TH_G,    KC_M,    KC_P,    TH_V,    TH_MINS, TH_DOT,  TH_SLSH, TH_DQUO, TH_QUOT, XXXXXXX, KC_PGDN,                   \
    ...
    * |-----------------------------------------------------------------------------------------+
    * |        |TabBk|WinBk|CpPst|CCCV |Brit |Vol+ |PG Up| UP  |Home | App |     |     |        |
    * |-----------------------------------------------------------------------------------------+
    * |         |     | GUI | Alt |Ctrl | Dim |Vol- |Left | DN  |Right|Menu |     |             |
    * |-----------------------------------------------------------------------------------------+
    * |           |Sleep|     |CapLk|PtScn|     |MPLY |Pg Dn|Insrt| End |     |          |Pg Up |
    * |-----------------------------------------------------------------------------------------+
  ...
  [_NAV] = LAYOUT_infinity( \
    ...
    XXXXXXX, TAB_BK,  WIN_BK,  CPY_PST, CC_CCCV, KC_BRIU, KC_VOLU, KC_PGUP, KC_UP,   KC_HOME, KC_APP,  XXXXXXX, XXXXXXX, XXXXXXX,          \
    XXXXXXX, XXXXXXX, KC_LGUI, KC_LALT, KC_LCTL, KC_BRID, KC_VOLD, KC_LEFT, KC_DOWN, KC_RGHT, KC_MENU, XXXXXXX, XXXXXXX,                   \
    XXXXXXX, KC_SLEP, XXXXXXX, KC_CAPS, KC_PSCR, XXXXXXX, TH_MPLY, KC_PGDN, KC_INS,  KC_END,  XXXXXXX, XXXXXXX, KC_PGUP,                   \
  ...
  [_NUM] = LAYOUT_infinity( \
    ...
    XXXXXXX, KC_HASH, KC_DLR,  KC_PERC, KC_CIRC, KC_EQL,  KC_MINS, TH_4,    TH_5,    TH_6,    KC_F12,  XXXXXXX, KC_PGDN,                   \
  ...
  [_CFG] = LAYOUT_infinity( \
    ...
    * |GESC |  1  |  2  |  3  |  4  |  5  |  6  |  7  |  8  |  9  |  0  |  -  |  =  |  \  |  ~  |
    * |-----------------------------------------------------------------------------------------+
    * |  Tab   |  Q  |Leadr| Esc | Esc |Bksp |Bksp | Tab | Tab |Leadr|  P  |  [  |  ]  | Bk Spc |
    * |-----------------------------------------------------------------------------------------+
    * | Cap Lk  |  A  |  S  |  D  |CapsW| Ent | Ent |CapsW|  K  |  L  |  ;  |  '  |    Enter    |
    * |-----------------------------------------------------------------------------------------+
    * |   Shift   |  Z  |  X  |Ctl Q|Ctl Q| Del | Del |Ctl Z|Ctl Z|  .  |  /  |  Shift   |Pg Dn |
    * |-----------------------------------------------------------------------------------------+
    * | Ctrl  |  GUI  |  Alt  |           Space                 |  Alt  | _NAV  | _NUM  | _CFG  |
    * `-----------------------------------------------------------------------------------------'
    */
...
/*
static bool process_tap_or_long_press_key(  // Handle calls to the Tap / Long Press function - The long press key code is tapped once
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {  // Handle calls to process_record_user
    ...
    switch (keycode) {  // Detect calls to custom keycodes
        ...
        case TH_B:  // KC_B on tap, KC_UNDS on long press
            return process_tap_or_long_press_key(record, (KC_UNDS));
        case TH_C:  // KC_C on tap, KC_BSLS on long press
            return process_tap_or_long_press_key(record, (KC_BSLS));
        case TH_F:  // KC_F on tap, Qu on long press
            if (record->event.pressed) {
                if ((mods | oneshot_mods) & MOD_MASK_SHIFT) {  // Was Shift held?
                    del_oneshot_mods(MOD_MASK_SHIFT);  // Thmporarily delete the Shift mod
                    unregister_mods(MOD_MASK_SHIFT);
            ...
            return false;
            break;
        case TH_G:  // KC_G on tap, KC_HASH on long press
            return process_tap_or_long_press_key(record, (KC_HASH));
        case TH_H:  // KC_H on tap, He on long press
            if (record->event.pressed) {
                if ((mods | oneshot_mods) & MOD_MASK_SHIFT) {  // Was Shift held?
                    del_oneshot_mods(MOD_MASK_SHIFT);  // Thmporarily delete the Shift mod
                    unregister_mods(MOD_MASK_SHIFT);
        ...
        case TH_K:  // KC_K on tap, Thorn on long press
            if (record->event.pressed) {
                if ((mods | oneshot_mods) & MOD_MASK_SHIFT) {  // Was Shift held?
                    del_oneshot_mods(MOD_MASK_SHIFT);  // Thmporarily delete the Shift mod
                    unregister_mods(MOD_MASK_SHIFT);
            ...
            return false;
            break;
        case TH_V:  // KC_V on tap, KC_AT on long press
            return process_tap_or_long_press_key(record, (KC_AT));
        case TH_W:  // KC_W on tap, KC_CIRC on long press
            return process_tap_or_long_press_key(record, (KC_CIRC));
        case TH_X:  // KC_X on tap, KC_Z on long press
            if (record->event.pressed) {
                if ((mods | oneshot_mods) & MOD_MASK_SHIFT) {  // Was Shift held?
                    del_oneshot_mods(MOD_MASK_SHIFT);  // Thmporarily delete the Shift mod
                    unregister_mods(MOD_MASK_SHIFT);
            ...
            return false;
            break;
        case TH_Y:  // KC_Y on tap, KC_AMPR on long press
            return process_tap_or_long_press_key(record, (KC_AMPR));
        ...
        case TH_DQUO:  // KC_DQUO on tap, KC_QUES on long press
        ...
        case TH_MPLY:  // KC_MPLY on tap, KC_MNXT on long press
            return process_tap_or_long_press_key(record, (KC_MNXT));
        case TH_QUOT:  // KC_QUOT on tap, KC_EXLM on long press
