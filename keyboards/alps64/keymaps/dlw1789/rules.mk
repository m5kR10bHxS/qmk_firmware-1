...
DEBOUNCE_TYPE = sym_defer_g            # Configure debounce algorithm to be used
ENCODER_ENABLE = no                    # Enable the use of one or more encoders
ENCODER_MAP_ENABLE = no                # Enable the use of the new encoder map configuration paradigm
...
RGBLIGHT_ENABLE = no                   # Enable addressable underglow RGB LEDs - Enabled in info.json
...
DEFERRED_EXEC_ENABLE = no              # Enable deferred execution for SM_TD (home row mods)
DYNAMIC_TAPPING_TERM_ENABLE = yes      # Allow tapping term adjustmet on the fly NOTE: Adjustments are not persistent
KEY_LOCK_ENABLE = no                   # Disable key lock feature
KEY_OVERRIDE_ENABLE = no               # Enable Key Override support - For custom shift keys
...
SPACE_CADET_ENABLE = no                # Disalbe the Space Cadet feature
